...
<template>
    <div>
         <span class="like-btn" @click="likeVacante" :class="{ 'like-active' : isActive }"></span>
                                                     <!--Coloree el corazon si el valor d elike es true-->

         <p class="mt-8">{{ cantidadLikes }} Likes</p>
...
<script>
    export default {
        props: ['vacanteId', 'like', 'likes'],  //Recordar que hay que poner aqui tods las variables asociadas a el componente
        data: function() {
            return {
                isActive: this.like,
        ...
        methods: {
            likeVacante() {
                axios.post('/vacanteslikes/' + this.vacanteId)
                    .then(respuesta => {
                        console.log('likes', respuesta)
